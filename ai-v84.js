...
// Plan -> Execute -> Reflect の最初のループを実装
// ========================================
...
function tpBuildTodayContext() {
    const events = tpGetTodayEvents();
    return events.map((e, i) => ({
        index: i,
        time: String(e?.time || ''),
        title: String(e?.title || '予定'),
        completed: !!e?.checked
    }));
}
...
function tpEnsureTodayAIStyles() {
    ...
    style.textContent = `
      .tp-today-ai { margin-bottom:20px; }
      .tp-today-ai-head { display:flex; align-items:center; gap:12px; }
      .tp-today-ai-icon { width:42px; height:42px; border-radius:13px; display:flex; align-items:center; justify-content:center; background:rgba(168,85,247,.15); color:var(--color-ai); font-size:18px; flex:none; }
      .tp-today-ai-title { font-size:15px; font-weight:900; }
      .tp-today-ai-sub { font-size:11px; color:var(--text-muted); margin-top:3px; }
      .tp-today-ai-result { margin-top:15px; padding:14px; border-radius:14px; background:rgba(168,85,247,.07); border:1px solid rgba(168,85,247,.18); display:none; }
      .tp-today-ai-label { font-size:10px; color:var(--text-muted); font-weight:800; margin-bottom:5px; }
      .tp-today-ai-main { font-size:18px; font-weight:900; }
      .tp-today-ai-time { font-size:13px; font-family:monospace; font-weight:900; margin-top:3px; }
      .tp-today-ai-reason { font-size:12px; color:var(--text-muted); line-height:1.55; margin-top:9px; }
      .tp-today-ai-empty { margin-top:12px; padding:11px 12px; border-radius:10px; background:rgba(128,128,128,.08); font-size:11px; color:var(--text-muted); line-height:1.5; }
    `;
...
function tpEnsureTodayAICard() {
    ...
    card.innerHTML = `
      <div class="tp-today-ai-head">
        <div class="tp-today-ai-icon"><i class="fa-solid fa-wand-magic-sparkles"></i></div>
        <div style="flex:1;">
          <div class="tp-today-ai-title">今日のAI提案</div>
          <div class="tp-today-ai-sub">今ある予定から「次にやること」を最適化</div>
        </div>
      </div>
      <button id="tp-today-ai-btn" class="btn-primary" type="button" style="margin-top:14px;padding:12px;font-size:13px;">
        <i class="fa-solid fa-brain"></i> 今日をAI分析
      </button>
      <div id="tp-today-ai-empty" class="tp-today-ai-empty" style="display:none;"></div>
      <div id="tp-today-ai-result" class="tp-today-ai-result">
        <div class="tp-today-ai-label">次にやること</div>
        <div id="tp-today-ai-main" class="tp-today-ai-main"></div>
        <div id="tp-today-ai-time" class="tp-today-ai-time"></div>
        <div class="tp-today-ai-label" style="margin-top:12px;">AIの理由</div>
        <div id="tp-today-ai-reason" class="tp-today-ai-reason"></div>
      </div>`;
...
function tpParseSuggestion(raw, context) {
    ...
    if (match) {
        try {
            ...
            if (parsed && (parsed.next_task || parsed.reason)) return parsed;
        } catch (_) {}
    }

    const first = context.find(e => !e.completed);
    return {
        next_task: first?.title || '今日の予定を確認',
        recommended_time: first?.time || '',
        reason: text || 'まず未完了の予定から1つ選んで始めるのがおすすめです。'
    };
}

async function tpAnalyzeToday() {
    if (!requireAILogin()) return;
    const card = tpEnsureTodayAICard();
    const btn = document.getElementById('tp-today-ai-btn');
    const empty = document.getElementById('tp-today-ai-empty');
    const result = document.getElementById('tp-today-ai-result');
    if (!btn || !card) return;
    ...
    if (!context.length) {
        ...
        empty.textContent = '今日は予定がありません。まずカレンダーから予定を1つ作成すると、AIが時間の使い方を提案できます。';
        return;
    }
    ...
    result.style.display = 'none';
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> 今日の予定を分析中...';

    const now = new Date();
    const nowText = now.toLocaleTimeString('ja-JP', { hour:'2-digit', minute:'2-digit', hour12:false });
    ...
    try {
        ...
        empty.style.display = 'block';
        empty.textContent = e?.message === 'LOGIN_REQUIRED' ? 'AI機能を利用するにはログインしてください。' : 'AIに接続できませんでした。少し時間を置いて再試行してください。';
    } finally {
        btn.disabled = false;
        btn.innerHTML = '<i class="fa-solid fa-brain"></i> 今日をAI分析';
    }
}
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    safeStorage.remove('tp_api_v800');
    updateAISettingsUI();
    setTimeout(tpEnsureTodayAICard, 300);
});
